...
class Wrapper implements ArrayAccess, Iterator {
	...
	public function format()
	{
		if(is_int($this->variable)) {
			...
			if(count($args) === 1 || $this->variable instanceof DateTime) {
				...
				return new Wrapper(number_format($this->variable, $args[0], 
						$args[1], $args[2]));
			}
		}
		return new Wrapper(vsprintf((string)$this->variable, func_get_args()));
	}
	...
	public function date($format)
	{
		...
		if($item instanceof DateTime) {
			...
			$tmp = new Wrapper(strftime(func_get_arg(0), $this->variable->format('U')));
			// BUG: cannot reliably reset timezone to previous, see PHP bug 45543
			// $this->variable->setTimezone($tz);
			return $tmp;
		} else if(is_int($item)) {
			return new Wrapper(strftime($format, $item));
		}
		return new Wrapper(strftime($format, time()));
	}
	...
	public function current()
	{
		if(is_array($this->variable)) {
			return new Wrapper(current($this->variable), $this->arrayIndex);
		}
		return $this;
	}
	...
	{
		if(is_array($this->variable)) {
			return new Wrapper(key($this->variable), $this->arrayIndex);
		}
	...
	public function next()
	{
		if(is_array($this->variable)) {
			$this->arrayIndex++;
			return new Wrapper(next($this->variable), $this->arrayIndex);
		}
	...
	public function offsetGet($offset)
	{
		if(is_array($this->variable) && isset($this->variable[$offset])) {
			if(is_int($offset)) {
				return new Wrapper($this->variable[$offset], $offset);
			}
			return new Wrapper($this->variable[$offset]);
